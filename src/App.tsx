...
import { sendScoreToGoogleSheet } from './utils/googleSheetSync';
...
export default function App() {
  ...
  const handleSaveGoogleSheetUrl = (url: string) => {
    ...
    try {
      if (url) {
        ...
        localStorage.removeItem('quiz_google_sheet_url');
      }
    } catch (e) {}
  };
  ...
  const calculateAndSubmitScore = () => {
    ...
    // Auto-sync score to Google Sheet if Web App URL is configured
    if (googleSheetUrl) {
      sendScoreToGoogleSheet(googleSheetUrl, {
        playerName: playerInfo.name || 'Thí sinh ẩn danh',
        className: playerInfo.className || 'Chưa có',
